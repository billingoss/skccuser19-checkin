...
기능적 요구사항
1. 고객이 흡연구역에서 체크인 한다.
1. 체크인 하면 흡연구역 상태를 사용중으로 변경(USE)한다.
1. 고객이 흡연구역에서 체크아웃 한다.
1. 체크아웃 되면 포인트를 적립하고 흡연구역 상태를 비었음(VACANT)로 변경한다.
1. 고객이 포인트를 사용한다.
...
3. 성능
    ...
    1. 체크아웃후 흡연구역 사용여부가 VACANT 로 변경되면 체크인 내역에 상태를 성공(SUCCESS) 로 업데이트 한다. SAGA
    1. 체크인/아웃에 따라 흡연구역 사용여부(USE/VACANT)를 areaList 에서 확인 가능하다. CQRS
